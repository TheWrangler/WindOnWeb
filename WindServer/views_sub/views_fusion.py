...
###################################Profile views#######################################
def profile(request):
    profiles = Profile.objects.order_by('-date_time')[:3]
    profiles = profiles[::-1]
    return render(request, 'fusion/profile.html',{'profiles':profiles})

def profile_update(request,fusion,station):
    if(station == '全站'):
        profiles = Profile.objects.filter(fusion=fusion).order_by('-date_time')[:3]
    else:
...
def profile_search(request,fusion,station,dt):
    if(station == '全站'):
        profiles = Profile.objects.filter(fusion=fusion,date_time__gte = dt).order_by('date_time')[:3]
    else:
